...
def DacDataGen(channel_num, cfil, single_bin=0, scale=2**13):
    ...
    d_freq_half0 = (
        np.exp(1j * np.random.uniform(0, 2 * np.pi, size=M)) * fil[1 : M + 1]
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="127.0.0.1",
        help="The IP address of the RFSoC2x2",
    )
    parser.add_argument(
        "--fpg",
        dest="fpg",
        type=str,
        default="firmware/rfsocdactut_2025-06-30_1113.fpg",
        help="The fpg file uploaded to the board.",
    )
    parser.add_argument(
        "--single-bin",
        dest="sbin",
        type=int,
        default=0,
        help="The bin number of the signal. 0 means all bins.",
    )
    parser.add_argument(
        "--bw",
        dest="bw",
        type=float,
        default=250.0,
        help="The bandwidth of the signal in MHz.",
    )
    parser.add_argument(
        "--dac-len",
        dest="daclen",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="FFT points on the ADC samples.",
    )
    parser.add_argument(
        "--adc-fs",
        dest="adcfs",
        type=float,
        default=500.0,
        help="ADC sampling rate in MSps.",
    )
    parser.add_argument(
        "--dac-fs",
        dest="dacfs",
        type=float,
        default=1000.0,
        help="DAC sampling rate in MSps.",
    )
    parser.add_argument(
        "--scale",
        dest="scale",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The npz file, which stores the data written into DAC. The key has to be `data`.",
    )
    parser.add_argument(
        "--skip-config",
        dest="skipconfig",
        action="store_true",
        default=False,
        help="Skip the FPGA and PLL config.",
    )
    ...
    if args.npz is not None:
        ...
        d_time = DacDataGen(
            dac_fft_len, cfil, single_bin=single_bin, scale=scale
        )
        data = d_time.real
