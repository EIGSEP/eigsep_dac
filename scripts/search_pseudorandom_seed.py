...
def main():
    ...
    p.add_argument(
        "--start", type=int, required=True, help="First seed to try"
    )
    p.add_argument(
        "--stop", type=int, required=True, help="Last seed (exclusive)"
    )
    ...
    for seed in range(args.start, args.stop):
        ...
        if (
            args.progress_every
            and (seed - args.start) % args.progress_every == 0
        ):
            print(
                f"... seed={seed} best_so_far={best_seed} score={best:.6e}",
                flush=True,
            )
